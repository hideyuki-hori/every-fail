...
- 静的ページ (`/`, `/dots` デフォルト, `/dots/<id>`, `/404`): 全 SSG → R2 から配信 (Cache API で edge cache)
...
| `cloudflare-r2-bucket` | ユーザー | R2 bucket name (HTML / assets 一括) |
...
- secret は `~/.ef/db` に保管しない (id 系は db、key 系は env)
- credential は R2 (S3 互換) のみ (Cloudflare R2 → Manage R2 API Tokens で発行)
...
URL とディレクトリ構造を直対応させる (R2 キーもこの構造をそのまま使う)。

```
<root>/dist/index.html        /
<root>/dist/dots/index.html   /dots (一覧)
<root>/dist/404.html          /404
...
- すべて Cloudflare R2 に格納 (HTML / partial.html / 404.html / sitemap.xml / assets)
- credential は R2 (S3 互換 API) 一系統に統一
- HTML 取得の初回は R2 origin fetch だが、Worker 側で Cache API (`caches.default`) を使い edge cache する → 2 回目以降は KV 同等のレイテンシ
- Worker binding は `STORE` (R2 単一)。旧 `PAGES` (KV) は廃止
...
URL の pathname をそのまま R2 キーに対応させる (Worker 側のルーティング変換ロジックが不要)。

```
index.html              /
dots/index.html         /dots (一覧)
dots/<id>/index.html    /dots/<id> (完全 HTML)
dots/<id>/partial.html  /dots/<id> (article 部分のみ、SPA 遷移時)
...
deploy ごとに自分の担当キーを全削除 → upload する。差分はとらない。

`dots/` prefix が `ef deploy` (一覧 = `dots/index.html`) と `ef dot deploy` (個別 = `dots/<id>/...`) で共有されるため、**prefix 一括削除は使わず明示的なキー単位 delete** で衝突を避ける。

| コマンド | 削除対象 (明示的キー) |
|---|---|
| `ef deploy` | `index.html` / `dots/index.html` / `404.html` / `sitemap.xml` (一覧のみ、`<id>/` は触らない) / `assets/*` |
| `ef dot deploy <id>` | `dots/<id>/index.html` / `dots/<id>/partial.html` / `dots/<id>/assets/*` |

R2 の `ListObjectsV2` で prefix 検索する場合は、`dots/<id>/` のように `<id>/` まで含めた prefix を使う (`dots/` だと一覧と衝突するため)。
